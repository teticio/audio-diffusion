# based on https://github.com/huggingface/diffusers/blob/main/examples/train_controlnet.py
...
def main(args):
    ...
    model = AudioDiffusionPipeline.from_pretrained(args.pretrained_model_name_or_path)
    noise_scheduler = model.scheduler
    vae = model.vqvae if getattr(model.config, "vqvae", None) is not None else None
    unet = model.unet
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = ControlNetModel.from_unet(
            unet, conditioning_channels=1, conditioning_embedding_out_channels=(128,)
        )
    ...
    # Move vae and unet and to device and cast to weight_dtype
    if vae is not None:
